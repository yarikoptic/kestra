...
public class JdbcQueueProvider implements AutoCloseable {
    ...
    public synchronized <T> ExclusiveVNodeJdbcQueue<T> exclusive(String queue, Class<T> type) {
        return (ExclusiveVNodeJdbcQueue<T>) queuesByName.computeIfAbsent(queue, (key) -> new ExclusiveVNodeJdbcQueue<>(
            dslContextWrapper,
            offsetManager,
    ...
    @SuppressWarnings("unchecked")
    public synchronized <T> BroadcastJdbcQueue<T> broadcast(String queue, Class<T> type) {
        return (BroadcastJdbcQueue<T>) queuesByName.computeIfAbsent(queue, (key) -> new BroadcastJdbcQueue<>(
            dslContextWrapper,
            offsetManager,
            jdbcQueueConfiguration,
            executorsUtils,
